        ...
        (match (get out (unwrap! (get-out-value wtx btc-receiver) ERR_NATIVE_FAILURE))
          out (if (>= (get value out) MIN_SATS)
            (let (
                ...
                (current-count (var-get processed-tx-count))
                (max-deposit (get max-deposit current-pool))
                (delta-c (contract-call? .usda-faktory-pool get-swap-quote sbtc-amount-to-user (some 0x00)))
                (dy-d (get dy delta-c))
              ...
              (if (>= min-amount-out fat-finger-out)
                    (match (as-contract (contract-call? .usda-faktory-pool swap-a-to-b sbtc-amount-to-user
                        min-amount-out
                    ))
        ...
        (match (get out (unwrap! (get-out-value wtx btc-receiver) ERR_NATIVE_FAILURE))
          out (if (>= (get value out) MIN_SATS)
            (let (
                ...
                (current-count (var-get processed-tx-count))
                (max-deposit (get max-deposit current-pool))
                (delta-c (contract-call? .pepe-faktory-pool get-swap-quote sbtc-amount-to-user (some 0x00)))
                (dy-d (get dy delta-c))
                (fat-finger-out (/ (* dy-d (- PRECISION max-slip-rate)) PRECISION))
              )
              (asserts! (<= sbtc-amount-to-user available-sbtc)
              ...
              (if (>= min-amount-out fat-finger-out)
                    (match (as-contract (contract-call? .pepe-faktory-pool swap-a-to-b sbtc-amount-to-user
                        min-amount-out
                    ))
                        delta (let ((amount-received (get dy delta)))
                                (if is-blaze   
